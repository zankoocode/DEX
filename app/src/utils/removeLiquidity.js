...
/**
 * getTokensAfterRemove: Calculates the amount of `Eth` and `ZCD` tokens
...
export const getTokensAfterRemove = async (
  ...
  try {
    ...
    // Get the total supply of `zankoocode` LP tokens
    ...
    // Similarly we also maintain a ratio for the `ZCD` tokens, so here in our case
    // Ratio is -> (amount of ZCD tokens sent back to the user / CD Token reserve) = (LP tokens withdrawn) / (total supply of LP tokens)
    // Then (amount of ZCD tokens sent back to the user) = (ZCD token reserve * LP tokens withdrawn) / (total supply of LP tokens)
    const _removeEther = _ethBalance.mul(removeLPTokenWei).div(_totalSupply);
    const _removeZCD = zankoocodeTokenReserve
